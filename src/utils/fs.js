  ...
  if (fs.existsSync(path)) { 
    ...
    files.forEach((file, index) => {
      if (Path.extname(file) === '.js') {
        ...
        let txt = file_content.split("export default ")[1].trim().RTrim(';').trim()
        let sheet = {
          name: file,
          value: eval('('+txt+')')
        }
...
function jsToExcel(READFLODER, DISTFILEPATH) {
  ...
  function createSheet(table) {
    ...
    Object.keys(table.value).map((ele, idx, arr) => {
...
async function excelToJs(READXSLTFILE,DISTFILEPATH) {
  ...
  workbook.eachSheet(function (worksheet, sheetId) {
    ...
    let keyColArr = []
    let valColArr = []
    keyCol.eachCell({ includeEmpty: true }, function(cell, rowNumber) {
      keyColArr.push(isNull(cell.value) ? '' : cell.value)
    });
    valCol.eachCell({ includeEmpty: true }, function(cell, rowNumber) {
      valColArr.push(isNull(cell.value) ? '' : cell.value )
    });
    let jsonObj = {}
    keyColArr.map((ele, idx, arr) => {
